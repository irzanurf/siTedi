

        <div id="page-wrapper">

<div class="container-fluid">

    <!-- Page Heading -->
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">
                Daftar Ketua Departemen
            </h1>
            <ol class="breadcrumb">
                <li>
                    <i class="fa fa-dashboard"></i>  <a href="<?= base_url('admin/dashboard/');?>">Dashboard</a>
                </li>
                <li class="active">
                    <i class="fa fa-edit"></i> Daftar Ketua Departemen
                </li>
            </ol>
        </div>
    </div>
    <!-- /.row -->
    <div class='row'>
    

    
    </div>

    <div class="row">
        <div class="col-lg-12">
        <section class="content">
        
        
        <a href="<?=base_url('admin/dashboard/tambahKadep');?>"><button class='btn btn-info'>Tambah Daftar Ketua Departemen</button></a>
        
        <table class="table">
            <tr>
                <th>NIDN</th>
                <th>Nama</th>
                <th>Departemen / Prodi</th>
                <th>Edit</th>
                <th>Hapus</th>

            </tr>
            <?php 
            $no = 1;
            foreach($view as $v) { ?>
            <tr>
                <td><?= $v->nip?></td>
                <td><?= $v->nama?></td>
                <td><?= $v->dep?></td>
                <td>
                
                <form method="post" action=<?= base_url('admin/dashboard/editKadep');?>>
                                    <input type='hidden' name="nip" value="<?= $v->nip ?>">
                                    <button type="submit" class="btn btn-success">
                                        Edit
                                    </button>
                                    
                                </form>
            </td>
            <td>
                <form method="post" onclick="return confirm('Apakah Anda Yakin Ingin Menghapus Data?');" action=<?= base_url('admin/dashboard/hapusKadep');?>>
                                    <input type='hidden' name="nip" value="<?= $v->nip ?>">
                                    <button type="submit" class="btn btn-danger">
                                        Hapus
                                    </button>
                                    
                                </form>
                </td>
                
                
            </tr>
            
            <?php } ?>
        </table>
        </section>
        

       
        

            

        </div>
        


        </div>
    </div>
    <!-- /.row -->

</div>
<!-- /.container-fluid -->

</div>
<!-- /#page-wrapper -->

</div>
<!-- /#wrapper -->

